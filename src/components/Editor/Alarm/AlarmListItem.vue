...
<template>
	...
	<div v-click-outside="closeAlarmEditor" class="property-alarm-item">
		<div class="property-alarm-item__icon">
			<div class="icon" :class="icon" />
		</div>
		<div v-if="!isEditing" class="property-alarm-item__label">
		...
		<div v-if="!isReadOnly" class="property-alarm-item__options">
			<Actions>
				<ActionRadio
					:name="alarmTypeName"
				...
				<ActionButton icon="icon-delete" @click="removeAlarm">
...
export default {
	...
	methods: {
		...
		changeRelativeHourMinuteAllDay(date) {
			...
			if (this.alarm.type === "AUDIO") {
				const dateTime = formatDate(
					new Date(),
					this.alarm.relativeAmountAllDay
				);
